# Command line генератор списков для MyHomeLib и вспомогательные утилиты.

##### Все программы, запущенные без ключей выводят справку о своих возможностях.

Примеры вызовов всех програм можно посмотреть в прилагающихся PowerShell (fb2_flibusta.ps1) или bash (fb2_flibusta.sh) скриптах. Все вместе утилиты позволяют накапливать библиотеки и создавать INPX индексы автоматически, практически без ручного вмешательста.
...
**libmerge** "накапливает" архив со специальным расширением .merging, вливая в него получаемые обновления пока он не достигнет заданного размера, после чего архив переименовывается в стандартный .zip (например fb2-474923-477023.zip). Делается это быстро, без переупаковки архивов. Пример работы:
