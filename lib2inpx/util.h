  ...
  public:
	...
	std::string passed()
	{
		return boost::posix_time::to_simple_string(
		    boost::posix_time::time_duration(boost::posix_time::second_clock::local_time() - m_start));
	}
...
class zip : boost::noncopyable {
  ...
  public:
	zip(const std::string& name, const std::string& comment = std::string(), bool create = true)
	    : m_comment(comment), m_opened(false), m_name(name), m_zf(NULL)
	{
		if (!m_func_set) {
			m_func_set = true;
#ifdef _WIN32
		...
		if (!m_opened) {
			if (UNZ_OK != (m_ziperr = unzOpenCurrentFile(m_unz.m_uf))) {
				throw std::runtime_error(
				    tmp_str("Problem reading archive unzOpenCurrentFile(%d) \"%s\"", m_ziperr, m_unz.m_name.c_str()));
			}
	...
	void close(bool do_not_throw = false)
	{
		...
		if (m_opened) {
			...
			if (UNZ_OK != (m_ziperr = unzCloseCurrentFile(m_unz.m_uf))) {
				if (!do_not_throw) {
					throw std::runtime_error(
					    tmp_str("Error reading archive unzCloseCurrentFile(%d) \"%s\"", m_ziperr, m_unz.m_name.c_str()));
				}
			}
		}
	}
...
class fb2_parser {
	...
	struct element_props {
		...
		element_props(const element_props& from)
		    : cardinality(from.cardinality), count(from.count), data_handler(from.data_handler), text(from.text),
