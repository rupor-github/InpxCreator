...
wget --progress=dot:mega --tries=3 --user-agent=Mozilla/5.0 --append-output=%CDIR%\log_%NAME%_archives.log --recursive --span-hosts --no-directories --no-parent --no-remove-listing --accept=f.[0-9]*-[0-9]*.zip,f.fb2.[0-9]*-[0-9]*.zip --directory-prefix=%1\%NAME% --no-clobber -e robots=off %SITE%/daily

forfiles /D +0 /P %1\%NAME% /M f.??????-??????.zip /C "cmd /c echo @path" 2>NUL >%TF%
forfiles /D +0 /P %1\%NAME% /M f.fb2.??????-??????.zip /C "cmd /c echo @path" 2>NUL >>%TF%
...
%CDIR%\lib2inpx.exe --db-name=%NAME% --process=fb2 --archives=%1;%1\%NAME% --clean-when-done %WDIR%

GOTO :EOF

:usage
echo Need path to local archives, for example: "fb2_flibusta.cmd c:\library\local"
echo If there is second parameter - daily archives and dumps would not be downloaded,
echo for example "fb2_flibusta.cmd c:\library\local skip"
